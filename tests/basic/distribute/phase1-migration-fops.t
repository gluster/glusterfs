...
do
    echo abc > $i;
    mv $i ${i}.txt #Hoping at least one file will hash to another subvolume after rename
    ...
    then
        ...
        setfattr -n trusted.distribute.migrate-data -v force ${i}.txt &
        break;
    fi
done
...
TEST [ ! -z $cached_file ];
EXPECT_WITHIN $MIGRATION_START_TIMEOUT "Y" has_phase1_attrs $cached_file
