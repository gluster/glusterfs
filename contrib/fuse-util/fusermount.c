...
static int
add_mount(const char *source, const char *mnt, const char *type,
          const char *opts)
{
...
static int
may_unmount(const char *mnt, int quiet)
{
    struct mntent *entp;
    FILE *fp;
...
check_is_mount_child(void *p)
{
    ...
    while ((entp = getmntent(fp)) != NULL) {
        ...
        if (entp->mnt_dir[0] == '/' && strcmp(entp->mnt_dir + 1, last) == 0) {
            found = 1;
            break;
        }
    }
...
clone_newns(void *a)
{
    ...
    return __clone2(check_is_mount_child, stack, sizeof(buf) / 2, CLONE_NEWNS,
                    a, NULL, NULL, NULL);
#else
...
static int
check_is_mount(const char *last, const char *mnt)
{
    pid_t pid, p;
    int status;
...
static int
chdir_to_parent(char *copy, const char **lastp)
{
    char *tmp;
    const char *parent;
    char buf[65536];
...
static int
unmount_fuse_locked(const char *mnt, int quiet, int lazy)
{
    char *copy;
    const char *last;
    int res;
...
static int
unmount_fuse(const char *mnt, int quiet, int lazy)
{
    int res;
...
strip_line(char *line)
{
    ...
    if (s != NULL)
        s[0] = '\0';
    for (s = line + strlen(line) - 1; s >= line && isspace((unsigned char)*s);
         s--)
        ;
    s[1] = '\0';
...
static void
parse_line(char *line, int linenum)
{
    int tmp;
...
static int
begins_with(const char *s, const char *beg)
{
...
static struct mount_flags mount_flags[] = {
    {"rw", MS_RDONLY, 0, 1},         {"ro", MS_RDONLY, 1, 1},
...
static int
add_option(char **optsp, const char *opt, unsigned expand)
{
    char *newopts;
    if (*optsp == NULL)
        newopts = strdup(opt);
...
static int
get_mnt_opts(int flags, char *opts, char **mnt_optsp)
{
    int i;
    size_t l;
...
static int
opt_eq(const char *s, unsigned len, const char *opt)
{
...
static int
get_string_opt(const char *s, unsigned len, const char *opt, char **val)
{
    int i;
...
static int
do_mount(const char *mnt, char **typep, mode_t rootmode, int fd,
         const char *opts, const char *dev, char **sourcep, char **mnt_optsp,
         off_t rootsize)
{
    int res;
    ...
    for (s = opts, d = optbuf; *s;) {
        ...
        if (begins_with(s, fsname_str)) {
            ...
            check_empty = 0;
        } else if (!begins_with(s, "fd=") && !begins_with(s, "rootmode=") &&
            ...
            if (opt_eq(s, len, "large_read")) {
                ...
                if (res == 0 &&
                    ...
                    fprintf(stderr,
                            "%s: note: 'large_read' mount option is deprecated "
                            "for %i.%i kernels\n",
                            progname, kmaj, kmin);
                    skip_option = 1;
                }
            ...
            if (getuid() != 0 && !user_allow_other &&
                ...
                fprintf(stderr,
                        "%s: option %.*s only allowed if 'user_allow_other' is "
    ...
    *d = '\0';
    ...
    sprintf(d, "fd=%i,rootmode=%o,user_id=%i,group_id=%i", fd, rootmode,
            getuid(), getgid());
...
static int
check_perm(const char **mntp, struct stat *stbuf, int *mountpoint_fd)
{
    int res;
    const char *mnt = *mntp;
...
static int
try_open(const char *dev, char **devp, int silent)
{
...
static int
try_open_fuse_device(char **devp)
{
    int fd;
    int err;
...
static int
check_fuse_device(char *devfd, char **devp)
{
    int res;
    char *devlink;
...
static int
mount_fuse(const char *mnt, const char *opts, char *devfd)
{
    int res;
    int fd;
    char *dev;
    ...
    if (getuid() != 0 && mount_max != -1) {
        ...
        if (mount_count >= mount_max) {
            fprintf(stderr,
                    "%s: too many FUSE filesystems mounted; mount_max=N can be "
                    "set in /etc/fuse.conf\n",
                    progname);
            goto fail_close_fd;
        }
...
main(int argc, char *argv[])
{
    ...
    static const struct option long_opts[] = {
        {"unmount", no_argument, NULL, 'u'}, {"lazy", no_argument, NULL, 'z'},
