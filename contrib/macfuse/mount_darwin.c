              ...
              pid_t *mnt_pid, int status_fd) /* Not used on OS X */
{
    ...
    if (!mountpoint) {
        gf_log("glustefs-fuse", GF_LOG_ERROR, "missing or invalid mount point");
        goto err;
    }
    ...
    if (chldf == SIG_ERR) {
        gf_log("glusterfs-fuse", GF_LOG_ERROR, "signal() returned SIG_ERR: %s",
               strerror(errno));
        goto err;
    }

    /* check for user<->kernel match. */
    ret = sysctlbyname(SYSCTL_OSXFUSE_VERSION_NUMBER, version, &version_len,
                       NULL, (size_t)0);
    if (ret != 0) {
        gf_log("glustefs-fuse", GF_LOG_ERROR,
               "sysctlbyname() returned error: %s", strerror(errno));
        goto err;
    }
    ...
    if (version_len != version_len_desired) {
        gf_log("glusterfs-fuse", GF_LOG_ERROR,
               "version length mismatch for OSXFUSE %s", version);
        ret = -1;
        goto err;
    }
    ...
    gf_log("glusterfs-fuse", GF_LOG_INFO, "OSXFUSE kext version supported %s",
           version);
    ...
    if (!dev) {
        for (r = 0; r < OSXFUSE_NDEVICES; r++) {
            ...
            if ((fd = open(devpath, O_RDWR)) < 0) {
                GFFUSE_LOGERR("failed to open device %s (%s)", devpath,
                              strerror(errno));
                goto err;
            }
            dev = devpath;
    ...
    fd = open(dev, O_RDWR);
    if (fd < 0) {
        GFFUSE_LOGERR("failed to open device %s (%s)", dev, strerror(errno));
        ret = -1;
        goto err;
    }
    ...
    if (pid == 0) {
        ...
        if (pid == 0) {
            ...
            if (asprintf(&opts, "%s,fssubtype=glusterfs", mnt_param) == -1) {
                GFFUSE_LOGERR("asprintf() error: %s", strerror(errno));
                ret = -1;
                goto err;
            }
