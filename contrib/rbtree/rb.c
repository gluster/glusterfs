...
rb_probe(struct rb_table *tree, void *item)
{
    ...
    n = pa[k - 1]->rb_link[da[k - 1]] = tree->rb_alloc->libavl_malloc(
        tree->rb_alloc, sizeof *n);
    if (n == NULL)
        return NULL;
...
rb_t_insert(struct rb_traverser *trav, struct rb_table *tree, void *item)
{
    ...
    if (p != NULL) {
        trav->rb_table = tree;
        trav->rb_node = ((struct rb_node *)((char *)p -
                                            offsetof(struct rb_node, rb_data)));
...
static void
copy_error_recovery(struct rb_node **stack, int height, struct rb_table *new,
                    rb_item_func *destroy)
{
...
struct rb_table *
rb_copy(const struct rb_table *org, rb_copy_func *copy, rb_item_func *destroy,
        struct libavl_allocator *allocator)
{
    ...
    for (;;) {
        while (x->rb_link[0] != NULL) {
            ...
            y->rb_link[0] = new->rb_alloc->libavl_malloc(new->rb_alloc,
                                                         sizeof *y->rb_link[0]);
            if (y->rb_link[0] == NULL) {
        ...
        for (;;) {
            ...
            if (x->rb_link[1] != NULL) {
                y->rb_link[1] = new->rb_alloc->libavl_malloc(
                    new->rb_alloc, sizeof *y->rb_link[1]);
                if (y->rb_link[1] == NULL) {
                    copy_error_recovery(stack, height, new, destroy);
                    return NULL;
                }
