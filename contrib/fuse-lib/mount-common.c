                   ...
                   const char *type, const char *opts)
{
    ...
    if (res == -1) {
        GFFUSE_LOGERR("%s: sigprocmask: %s", progname, strerror(errno));
        return -1;
    }
    ...
    if (res == 0) {
        ...
        tmp = mkdtemp(templ);
        if (!tmp) {
            GFFUSE_LOGERR("%s: failed to create temporary directory", progname);
            exit(1);
        }
        if (chdir(tmp)) {
            GFFUSE_LOGERR("%s: failed to chdir to %s: %s", progname, tmp,
                          strerror(errno));
            exit(1);
        }
        rmdir(tmp);
        execl(_PATH_MOUNT, _PATH_MOUNT, "-i", "-f", "-t", type, "-o", opts,
              fsname, mnt, NULL);
        GFFUSE_LOGERR("%s: failed to execute %s: %s", progname, _PATH_MOUNT,
                      strerror(errno));
        exit(1);
    }
                ...
                int lazy)
{
    ...
    if (!mtab_needs_update(abs_mnt)) {
        ...
        if (res == -1)
            GFFUSE_LOGERR("%s: failed to unmount %s: %s", progname, abs_mnt,
                          strerror(errno));
        return res;
    }
...
#ifdef GF_LINUX_HOST_OS
        execl("umount", "umount", "-i", rel_mnt, lazy ? "-l" : NULL, NULL);
        GFFUSE_LOGERR("%s: failed to execute umount: %s", progname,
                      strerror(errno));
#elif __NetBSD__
