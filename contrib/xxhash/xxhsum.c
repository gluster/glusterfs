...
#define WELCOME_MESSAGE(exename)                                               \
    "%s %s (%i-bits %s), by %s \n", exename, PROGRAM_VERSION, g_nbBits,        \
        ENDIAN_NAME, author
...
#define TIMELOOP_S 1
#define TIMELOOP                                                               \
    (TIMELOOP_S * CLOCKS_PER_SEC) /* Minimum timing per iteration */
...
static const algoType g_defaultAlgo =
    algo_xxh64; /* required within main() & usage() */
 ...
 **************************************/
static clock_t
BMK_clockSpan(clock_t start)
{
    return clock() -
           start; /* works even if overflow; Typical max span ~ 30 mn */
}
...
static U64
BMK_GetFileSize(const char *infilename)
{
    int r;
#if defined(_MSC_VER)
...
static U32
localXXH32(const void *buffer, size_t bufferSize, U32 seed)
{
    return XXH32(buffer, bufferSize, seed);
}

static U32
localXXH64(const void *buffer, size_t bufferSize, U32 seed)
{
    return (U32)XXH64(buffer, bufferSize, seed);
}

static void
BMK_benchHash(hashFunction h, const char *hName, const void *buffer,
              size_t bufferSize)
{
    U32 nbh_perIteration = ((300 MB) / (bufferSize + 1)) +
                           1; /* first loop conservatively aims for 300 MB/s */
    U32 iterationNb;
    ...
    for (iterationNb = 1; iterationNb <= g_nbIterations; iterationNb++) {
        ...
        DISPLAYLEVEL(2, "%1i-%-17.17s : %10u ->\r", iterationNb, hName,
                     (U32)bufferSize);
        cStart = clock();
        while (clock() == cStart)
            ; /* starts clock() at its exact beginning */
        cStart = clock();
        ...
        if (r == 0)
            DISPLAYLEVEL(3, ".\r"); /* do something with r to avoid compiler
                                       "optimizing" away hash function */
        {
            double const timeS = ((double)BMK_clockSpan(cStart) /
                                  CLOCKS_PER_SEC) /
                                 nbh_perIteration;
            if (timeS < fastestH)
                fastestH = timeS;
        ...
        nbh_perIteration =
            (U32)(1 / fastestH) +
            1; /* adjust nbh_perIteration to last roughtly one second */
    }
    DISPLAYLEVEL(1, "%-19.19s : %10u -> %8.0f it/s (%7.1f MB/s) \n", hName,
                 (U32)bufferSize, (double)1 / fastestH,
                 ((double)bufferSize / (1 << 20)) / fastestH);
    if (g_displayLevel < 1)
...
static int
BMK_benchMem(const void *buffer, size_t bufferSize, U32 specificTest)
{
    ...
    if ((specificTest == 0) | (specificTest == 2))
        BMK_benchHash(localXXH32, "XXH32 unaligned", ((const char *)buffer) + 1,
                      bufferSize);
    ...
    if ((specificTest == 0) | (specificTest == 4))
        BMK_benchHash(localXXH64, "XXH64 unaligned", ((const char *)buffer) + 3,
                      bufferSize);
...
static size_t
BMK_selectBenchedSize(const char *fileName)
{
...
static int
BMK_benchFiles(const char **fileNamesTable, int nbFiles, U32 specificTest)
{
    int result = 0;
    int fileIdx;
    ...
    for (fileIdx = 0; fileIdx < nbFiles; fileIdx++) {
        ...
        void *const alignedBuffer = (buffer + 15) -
                                    (((size_t)(buffer + 15)) &
                                     0xF); /* align on next 16 bytes */
        ...
        {
            size_t const readSize = fread(alignedBuffer, 1, benchedSize,
                                          inFile);
            fclose(inFile);
...
static int
BMK_benchInternal(size_t keySize, int specificTest)
{
    void *const buffer = calloc(keySize + 16 + 3, 1);
    void *const alignedBuffer = ((char *)buffer + 15) -
                                (((size_t)((char *)buffer + 15)) &
                                 0xF); /* align on next 16 bytes */
    if (!buffer) {
        DISPLAY("\nError: not enough memory!\n");
        return 12;
...
static void
BMK_checkResult64(U64 r1, U64 r2)
{
    static int nbTests = 1;
    if (r1 != r2) {
...
static void
BMK_testSequence64(void *sentence, size_t len, U64 seed, U64 Nresult)
{
    XXH64_state_t state;
    U64 Dresult;
    size_t pos;
...
static void
BMK_testSequence(const void *sequence, size_t len, U32 seed, U32 Nresult)
{
    XXH32_state_t state;
    U32 Dresult;
    size_t pos;
...
BMK_sanityCheck(void)
{
    ...
    BMK_testSequence64(sanityBuffer, SANITY_BUFFER_SIZE, 0,
                       0x0EAB543384F878ADULL);
    BMK_testSequence64(sanityBuffer, SANITY_BUFFER_SIZE, prime,
...
static void
BMK_display_LittleEndian(const void *ptr, size_t length)
{
    const BYTE *p = (const BYTE *)ptr;
    size_t idx;
    for (idx = length - 1; idx < length;
         idx--) /* intentional underflow to negative to detect end */
        DISPLAYRESULT("%02x", p[idx]);
}

static void
BMK_display_BigEndian(const void *ptr, size_t length)
{
    const BYTE *p = (const BYTE *)ptr;
    size_t idx;
...
static void
BMK_hashStream(void *xxhHashValue, const algoType hashType, FILE *inFile,
...
static int
BMK_hash(const char *fileName, const algoType hashType,
         const endianess displayEndianess)
{
    ...
    /* loading notification */
    {
        ...
        const int maxInfoFilenameSize = (int)(fileNameSize > 30 ? 30
                                                                : fileNameSize);
        ...
        free(buffer);
        DISPLAY("%s             \r",
                fileNameEnd - infoFilenameSize); /* erase line */
    }
    ...
    switch (hashType) {
        case algo_xxh32: {
            ...
            displayEndianess == big_endian
                ? BMK_display_BigEndian(&hcbe32, sizeof(hcbe32))
                : BMK_display_LittleEndian(&hcbe32, sizeof(hcbe32));
            DISPLAYRESULT("  %s\n", fileName);
            break;
        }
        case algo_xxh64: {
            ...
            displayEndianess == big_endian
                ? BMK_display_BigEndian(&hcbe64, sizeof(hcbe64))
                : BMK_display_LittleEndian(&hcbe64, sizeof(hcbe64));
            DISPLAYRESULT("  %s\n", fileName);
            break;
        }
        default:
...
static int
BMK_hashFiles(const char **fnList, int fnTotal, algoType hashType,
              endianess displayEndianess)
{
    int fnNb;
    int result = 0;
...
/*  Read line from stream.
    ...
    Returns GetLine_exceedMaxLineLength, if line length is longer than
   MAX_LINE_LENGTH. Returns GetLine_outOfMemory, if line buffer memory
   allocation failed.
 */
static GetLineResult
getLine(char **lineBuf, int *lineMax, FILE *inFile)
{
    ...
    for (;;) {
        ...
        if (len + 1 >= *lineMax) {
            ...
            if (newBufSize > MAX_LINE_LENGTH)
                newBufSize = MAX_LINE_LENGTH;
            if (len + 1 >= newBufSize)
...
/*  Converts XXH32 canonical hexadecimal string hashStr to big endian unsigned
 * char array dst. Returns CANONICAL_FROM_STRING_INVALID_FORMAT, if hashStr is
 * not well formatted. Returns CANONICAL_FROM_STRING_OK, if hashStr is parsed
 * successfully.
 */
static CanonicalFromStringResult
canonicalFromString(unsigned char *dst, size_t dstSize, const char *hashStr)
{
    size_t i;
    for (i = 0; i < dstSize; ++i) {
        int h0, h1;
 ...
 */
static ParseLineResult
parseLine(ParsedLine *parsedLine, const char *line)
{
    ...
    switch (firstSpace - line) {
        case 8: {
            ...
            if (canonicalFromString(xxh32c->digest, sizeof(xxh32c->digest),
                                    line) != CanonicalFromString_ok) {
                return ParseLine_invalidFormat;
            }
        ...
        case 16: {
            ...
            if (canonicalFromString(xxh64c->digest, sizeof(xxh64c->digest),
                                    line) != CanonicalFromString_ok) {
                return ParseLine_invalidFormat;
            }
 ...
 */
static void
parseFile1(ParseFileArg *parseFileArg)
{
    ...
    while (!report->quit) {
        ...
        if (getLineResult != GetLine_ok) {
            ...
            switch (getLineResult) {
                case GetLine_ok:
                case GetLine_eof:
                    /* These cases never happen.  See above getLineResult
                       related "if"s. They exist just for make gcc's
                       -Wswitch-enum happy. */
                    break;
        ...
        if (report->xxhBits != 0 && report->xxhBits != parsedLine.xxhBits) {
            ...
            if (parseFileArg->warn) {
                DISPLAY(
                    "%s : %lu: improperly formatted XXHASH checksum line "
                    "(XXH32/64)\n",
                    inFileName, lineNumber);
            }
            continue;
        ...
        if (fp == NULL) {
            ...
            switch (parsedLine.xxhBits) {
                case 32: {
                    XXH32_hash_t xxh;
                    BMK_hashStream(&xxh, algo_xxh32, fp, parseFileArg->blockBuf,
                                   parseFileArg->blockSize);
                    if (xxh ==
                        XXH32_hashFromCanonical(&parsedLine.canonical.xxh32)) {
                ...
                case 64: {
                    XXH64_hash_t xxh;
                    BMK_hashStream(&xxh, algo_xxh64, fp, parseFileArg->blockBuf,
                                   parseFileArg->blockSize);
                    if (xxh ==
                        XXH64_hashFromCanonical(&parsedLine.canonical.xxh64)) {
                        lineStatus = LineStatus_hashOk;
                    }
                } break;
 ...
 *    - (strict mode) All lines in checksum file are consistent and well
 * formatted.
 *
 */
static int
checkFile(const char *inFileName, const endianess displayEndianess,
          U32 strictMode, U32 statusOnly, U32 warn, U32 quiet)
{
    int result = 0;
    FILE *inFile = NULL;
...
static int
checkFiles(const char **fnList, int fnTotal, const endianess displayEndianess,
           U32 strictMode, U32 statusOnly, U32 warn, U32 quiet)
{
    int ok = 1;
    ...
    if (fnTotal == 0) {
        ok &= checkFile(stdinName, displayEndianess, strictMode, statusOnly,
                        warn, quiet);
    } else {
        int fnNb;
        for (fnNb = 0; fnNb < fnTotal; fnNb++)
            ok &= checkFile(fnList[fnNb], displayEndianess, strictMode,
                            statusOnly, warn, quiet);
    }
    return ok ? 0 : 1;
}
...
usage_advanced(const char *exename)
{
    ...
    DISPLAY("Advanced :\n");
    DISPLAY(
        " --little-endian : hash printed using little endian convention "
        "(default: big endian)\n");
...
main(int argc, const char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
        ...
        if (*argument != '-') {
            if (filenamesStart == 0)
                filenamesStart =
                    i; /* only supports a continuous list of filenames */
            continue;
        }
        ...
        while (*argument != 0) {
            switch (*argument) {
                ...
                case 'b':
                    argument++;
                    benchmarkMode = 1;
                    specificTest = readU32FromChar(
                        &argument); /* select one specific test (hidden option)
                                     */
                    break;
    ...
    if (benchmarkMode) {
        ...
        BMK_sanityCheck();
        if (filenamesStart == 0)
            return BMK_benchInternal(keySize, specificTest);
        return BMK_benchFiles(argv + filenamesStart, argc - filenamesStart,
                              specificTest);
    }

    /* Check if input is defined as console; trigger an error in this case */
    if ((filenamesStart == 0) && IS_CONSOLE(stdin))
        return badusage(exename);
    ...
    if (fileCheckMode) {
        return checkFiles(argv + filenamesStart, argc - filenamesStart,
                          displayEndianess, strictMode, statusOnly, warn,
                          quiet);
    } else {
        return BMK_hashFiles(argv + filenamesStart, argc - filenamesStart, algo,
                             displayEndianess);
    }
}
