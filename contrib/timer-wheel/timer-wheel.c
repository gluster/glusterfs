...
unsigned long
gf_tw_find_last_bit(const unsigned long *, unsigned long);

#if defined(__GNUC__) || defined(__clang__)
static inline unsigned long
gf_tw_fls(unsigned long word)
{
    return BITS_PER_LONG - __builtin_clzl(word);
}
#else
extern unsigned long
gf_tw_fls(unsigned long);
#endif
...
static inline int
timer_pending(struct gf_tw_timer_list *timer)
{
    struct list_head *entry = &timer->entry;
...
static inline int
__detach_if_pending(struct gf_tw_timer_list *timer)
{
...
static inline int
__mod_timer(struct tvec_base *base, struct gf_tw_timer_list *timer,
            int pending_only)
{
    int ret = 0;
 ...
 */
void
gf_tw_add_timer(struct tvec_base *base, struct gf_tw_timer_list *timer)
{
    pthread_spin_lock(&base->lock);
    {
 ...
 */
int
gf_tw_del_timer(struct tvec_base *base, struct gf_tw_timer_list *timer)
{
    int ret = 0;
...
int
gf_tw_mod_timer_pending(struct tvec_base *base, struct gf_tw_timer_list *timer,
                        unsigned long expires)
{
    int ret = 1;
...
int
gf_tw_mod_timer(struct tvec_base *base, struct gf_tw_timer_list *timer,
                unsigned long expires)
{
    int ret = 1;
...
int
gf_tw_cleanup_timers(struct tvec_base *base)
{
    int ret = 0;
    void *res = NULL;
