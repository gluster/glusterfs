...
def main():
    ...
    p.add_argument("--main-dist-count", type=int,
                   help="Main Distribution count")
    ...
    if config_file is not None and \
       args.subcmd in ["monitor", "config-get", "config-set", "config-reset"]:
        ret = gconf.is_config_file_old(config_file, args.main, extra_tmpl_args["subordinatevol"])
        if ret is not None:
