...
{
  gf_log  ("libglusterfs", GF_LOG_DEBUG, "conf.y: set_chroot_dir: chroot directory = %s", dir);
...
{
  gf_log ("libglusterfs", GF_LOG_DEBUG, "conf.y: set_scratch_dir: scratch directory = %s", dir);
...
{
  gf_log ("libglusterfs", GF_LOG_DEBUG, "conf.y: set_key_len: maximum key length = %s", key);
...
{
  gf_log ("libglusterfs", GF_LOG_DEBUG, "conf.y: set_inet_port: interconnect protocol is %s", prot);
...
ccerror (const char *str)
{
  parse_error ();
  complete_confd = NULL;
  gf_log ("libglusterfs", GF_LOG_DEBUG, "conf.y: ccerror: %s", str);
  return 0;
}
