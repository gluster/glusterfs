...
int
afr_sh_entry_trim_cbk (call_frame_t *frame, void *cookie, xlator_t *this,
...
int
afr_sh_entry_trim_sinks (call_frame_t *frame, xlator_t *this)
{
	afr_private_t * priv = NULL;
	afr_local_t * local  = NULL;
	afr_self_heal_t *sh  = NULL;
	...
	for (i = 0; i < priv->child_count; i++) {
		...
		STACK_WIND_COOKIE (frame, afr_sh_entry_trim_cbk,
				   (void *) (long) i,
				   priv->children[i], 
...
int
afr_sh_entry_read_write (call_frame_t *frame, xlator_t *this);

int
afr_sh_entry_write_cbk (call_frame_t *frame, void *cookie, xlator_t *this, 
			int32_t op_ret, int32_t op_errno, struct stat *buf)
{
	afr_private_t * priv = NULL;
	afr_local_t * local  = NULL;
	...
	if (call_count == 0) {
		...
		if (sh->offset < sh->file_size) {
			afr_sh_entry_read_write (frame, this);
		} else {
			gf_log (this->name, GF_LOG_DEBUG, 
				"closing fd's");
		
			afr_sh_entry_trim_sinks (frame, this);
		}
	}
...
int
afr_sh_entry_read_cbk (call_frame_t *frame, void *cookie,
		       ...
		       struct iovec *vector, int32_t count, struct stat *buf)
{
	...
	for (i = 0; i < priv->child_count; i++) {
		...
		STACK_WIND_COOKIE (frame, afr_sh_entry_write_cbk,
				   (void *) (long) i,
				   priv->children[i],
...
afr_sh_entry_read_write (call_frame_t *frame, xlator_t *this)
{
	...
	STACK_WIND_COOKIE (frame, afr_sh_entry_read_cbk,
			   (void *) (long) sh->source,
...
int
afr_sh_entry_open_cbk (call_frame_t *frame, void *cookie, xlator_t *this,
...
afr_sh_entry_open (call_frame_t *frame, xlator_t *this)
{
	...
	/* open source */
	STACK_WIND_COOKIE (frame, afr_sh_entry_open_cbk,
			   (void *) (long) source,
