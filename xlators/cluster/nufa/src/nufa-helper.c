...
int
nufa_frame_return (call_frame_t *frame)
{
...
int
nufa_itransform (xlator_t *this, xlator_t *subvol, uint64_t x, uint64_t *y_p)
{
	...
	cnt = nufa_subvol_cnt (this, subvol);
...
int
nufa_deitransform (xlator_t *this, uint64_t y, xlator_t **subvol_p,
		  uint64_t *x_p)
{
	nufa_conf_t *conf = NULL;
	int         cnt = 0;
...
void
nufa_local_wipe (nufa_local_t *local)
{
	if (!local)
		return;
...
nufa_local_t *
nufa_local_init (call_frame_t *frame)
{
	nufa_local_t *local = NULL;
...
nufa_subvol_get_hashed (xlator_t *this, loc_t *loc)
{
        ...
        layout = nufa_layout_get (this, loc->parent);
        ...
        subvol = nufa_layout_search (this, layout, loc->name);
...
xlator_t *
nufa_subvol_get_cached (xlator_t *this, inode_t *inode)
{
        nufa_layout_t *layout = NULL;
        xlator_t     *subvol = NULL;
...
xlator_t *
nufa_subvol_next (xlator_t *this, xlator_t *prev)
{
	nufa_conf_t *conf = NULL;
	int         i = 0;
	xlator_t   *next = NULL;
...
int
nufa_subvol_cnt (xlator_t *this, xlator_t *subvol)
{
	int i = 0;
	int ret = -1;
	nufa_conf_t *conf = NULL;
...
int
nufa_stat_merge (xlator_t *this, struct stat *to,
		struct stat *from, xlator_t *subvol)
{
	to->st_dev      = from->st_dev;

	nufa_itransform (this, subvol, from->st_ino, &to->st_ino);
