...
int
nufa_hash_compute_internal (int type, const char *name, uint32_t *hash_p)
{
	int      ret = 0;
	uint32_t hash = 0;
...
int
nufa_hash_compute (int type, const char *name, uint32_t *hash_p)
{
	char     *rsync_friendly_name = NULL;

	MAKE_RSYNC_FRIENDLY_NAME (rsync_friendly_name, name);

	return nufa_hash_compute_internal (type, rsync_friendly_name, hash_p);
}
