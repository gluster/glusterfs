	       ...
	       dict_t *dict)
{
  ...
  int child_count = pvt->child_count, i, callcnt;
  data_t *state_data;
  char *state;
  ...
  if (callcnt == 0) {
    ...
    if (local->revalidate_error == 1) {
      local->op_ret = -1;
      local->op_errno = EIO;
      GF_DEBUG (this, "revalidate error on");
    }
    STACK_UNWIND (frame,
		  local->op_ret,
		  local->op_errno,
	 ...
	 loc_t *loc)
{
  ...
  if (local == NULL) {
    ...
    local->state = data_to_bin (dict_get(loc->inode->ctx, this->name));
		     ...
		     dict_t *dict)
{
  ...
  stateino = data_to_ptr (dict_get (local->stub->args.mknod.loc.inode->ctx, this->name));  
...
fini (xlator_t *this)
{
  ha_private_t *priv = this->private;
  FREE (priv);
  return;
}
