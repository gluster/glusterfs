...
svs_lookup(call_frame_t *frame, xlator_t *this, loc_t *loc, dict_t *xdata)
{
    ...
    root = frame->root;
    ret = gf_setcredentials(&root->uid, &root->gid, root->ngrps, root->groups);
    if (ret < 0) {
        goto out;
    }
             ...
             dict_t *xdata)
{
    ...
    */
    if (inode_ctx->type == SNAP_VIEW_ENTRY_POINT_INODE) {
        ...
        if (name) {
            ret = dict_set_dynptr(dict, (char *)name, value, size);
            if (ret < 0) {
                op_ret = gf_error;
                op_errno = -ret;
                gf_msg(this->name, GF_LOG_ERROR, op_errno,
            ...
            ret = svs_add_xattrs_to_dict(this, dict, value, size);
            if (ret < 0) {
                op_ret = gf_error;
                op_errno = ENOMEM;
                gf_msg(this->name, GF_LOG_ERROR, op_errno, SVS_MSG_NO_MEMORY,
             ...
             dict_t *xdata)
{
    ...
    buf = alloca(size + 1);
    ret = glfs_h_readlink(fs, object, buf, size);
    SET_RET(op_ret, ret);
    if (ret < 0) {
        op_errno = errno;
