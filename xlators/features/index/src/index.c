...
index_get_type_from_vgfid(index_priv_t *priv, uuid_t vgfid)
{
    ...
    for (i = 0; i < XATTROP_TYPE_END; i++) {
        if (uuid_compare(priv->internal_vgfid[i], vgfid) == 0)
            return i;
    }
    return -1;
}
...
index_is_subdir_of_entry_changes(xlator_t *this, inode_t *inode)
{
    ...
    if ((ret == 0) && !uuid_is_null(ctx->virtual_pargfid))
        return _gf_true;
    return _gf_false;
}
...
index_is_fop_on_internal_inode(xlator_t *this, inode_t *inode, uuid_t gfid)
{
    ...
    if (gfid && !uuid_is_null(gfid))
        uuid_copy(vgfid, gfid);
    else
        uuid_copy(vgfid, inode->gfid);
...
index_entry_create(xlator_t *this, inode_t *inode, char *filename)
{
    ...
    GF_ASSERT_AND_GOTO_WITH_ERROR(this->name, !uuid_is_null(inode->gfid), out,
                                  op_errno, EINVAL);
...
index_entry_delete(xlator_t *this, uuid_t pgfid, char *filename)
{
    ...
    GF_ASSERT_AND_GOTO_WITH_ERROR(this->name, !uuid_is_null(pgfid), out,
                                  op_errno, EINVAL);
    GF_ASSERT_AND_GOTO_WITH_ERROR(this->name, filename, out, op_errno, EINVAL);
...
index_inode_path(xlator_t *this, inode_t *inode, char *dirpath, size_t len)
{
    ...
    if (subdir) {
        ...
        if (ret)
            goto out;
        if (uuid_is_null(ictx->virtual_pargfid)) {
            ret = -EINVAL;
            goto out;
        }
...
index_save_pargfid_for_entry_changes(xlator_t *this, loc_t *loc, char *path)
{
    ...
    if (!loc)
        return -1;
    if (uuid_compare(loc->pargfid, priv->internal_vgfid[ENTRY_CHANGES]))
        return 0;
    ...
    ret = uuid_parse(loc->name, ctx->virtual_pargfid);
    if (ret) {
        gf_msg(this->name, GF_LOG_ERROR, EINVAL,
               INDEX_MSG_INODE_CTX_GET_SET_FAILED,
               "Unable to store "
...
index_get_gfid_type(void *opaque)
{
    ...
    list_for_each_entry(entry, &args->entries->list, list)
    {
        ...
        if (uuid_parse(entry->d_name, loc.gfid))
            continue;
                      ...
                      int32_t *op_ret, int32_t *op_errno)
{
    ...
    uuid_copy(parent->ia_gfid, loc->pargfid);
    parent->ia_ino = -1;
                     ...
                     dict_t *xdata)
{
    ...
    if (type <= XATTROP_TYPE_UNSET) {
        ...
        if ((ret == 0) && uuid_is_null(ictx->virtual_pargfid)) {
            ret = -EINVAL;
        }
        if (ret == 0) {
...
init(xlator_t *this)
{
    ...
    uuid_generate(priv->index);
    for (i = 0; i < XATTROP_TYPE_END; i++)
        uuid_generate(priv->internal_vgfid[i]);
