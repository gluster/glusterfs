                    ...
                    struct iatt *statpre, struct iatt *statpost, dict_t *xdata)
{
        ...
        if (ret == -1) {
                ...
                gf_msg (this->name, GF_LOG_ERROR, -ret, P_MSG_FALLOCATE_FAILED,
                        "fallocate failed on %s offset: %"PRIu64", "
                        "len:%"GF_PRI_SIZET", flags: %d", uuid_utoa (fd->inode->gfid),
                        offset, len, flags);
                goto out;
        }
...
out:
        ...
        STACK_UNWIND_STRICT (readlink, frame, op_ret, op_errno, dest,
                             &stbuf, NULL);
...
__posix_pwritev (int fd, struct iovec *vector, int count, off_t offset)
{
        ...
        for (idx = 0; idx < count; idx++) {
                retval = sys_pwrite (fd, vector[idx].iov_base,
                                     vector[idx].iov_len, internal_off);
                if (retval == -1) {
                        op_ret = -errno;
                        goto err;
                }
             ...
             fd_t *fd, int32_t datasync, dict_t *xdata)
{
	...
	priv = this->private;
        ...
        if (priv->batch_fsync_mode && xdata &&
            dict_get (xdata, "batch-fsync")) {
...
map_xattr_flags(int flags)
{
        ...
        */
        int darwinflags = flags & ~(GF_XATTR_CREATE | GF_XATTR_REPLACE |
                                    XATTR_REPLACE);
...
int
static posix_readdirp_fill (xlator_t *this, fd_t *fd, gf_dirent_t *entries,
                            dict_t *dict)
{
